...
def svm_loss(x, y):
    ...
    loss = np.maximum(0, x - x[np.arange(x.shape[0]),y].reshape(-1,1) + 1)
    loss[np.arange(x.shape[0]),y] = 0
...
def softmax_loss(x, y):
    ...
    score = x / np.sum(x, axis = 1).reshape(-1,1)
    loss = -np.sum(np.log(1e-6+score[np.arange(num_train), y]))/num_train
