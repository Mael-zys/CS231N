...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        scorei = torch.zeros(2*N,1)
        scorej = torch.zeros(2*N,1)
        for l in range(2*N):
          scorei[l] = sim(z_k, out[l]) / tau
          scorej[l] = sim(z_k_N, out[l]) / tau
        scorei = torch.exp(scorei)
        scorej = torch.exp(scorej)
        scorei[k] = 0
        scorej[k+N] = 0
        total_loss += -torch.log(scorei[k+N] / torch.sum(scorei)) -torch.log(scorej[k] / torch.sum(scorej))
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (out_left/torch.linalg.norm(out_left, axis = 1).view(-1,1)) @ (out_right/torch.linalg.norm(out_right, axis = 1).view(-1,1)).T
    pos_pairs = torch.diag(pos_pairs).view(-1, 1)
...
def compute_sim_matrix(out):
    ...
    out_temp = out / torch.linalg.norm(out, axis = 1).view(-1,1)
    sim_matrix = out_temp @ out_temp.T
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = torch.sum(exponential, axis = 1).view(-1,1)
    ...
    sim_positive = sim_positive_pairs(out_left,out_right)
