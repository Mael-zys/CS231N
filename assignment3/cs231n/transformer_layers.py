...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        position = torch.arange(0,max_len).view(max_len,1)
        exp = torch.exp(torch.arange(0,embed_dim,2)*-math.log(10000)/embed_dim)
        pe[:,:,::2] = torch.sin(position * exp)
        pe[:,:,1::2] = torch.cos(position * exp)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        # also apply dropout just before the softmax. You may  #
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        k = self.key(key).view(N,T,H,D//H).permute(0,2,3,1) # N,H,D//H,T
        v = self.value(value).view(N,T,H,D//H).permute(0,2,1,3) # N,H,T,D//H

        e = q @ k / self.scale # N,H,S,T

        if attn_mask != None:
          e = e.masked_fill(attn_mask==0, float('-inf'))

        a = self.softmax(e) # N,H,S,T
        
        a = self.dropout(a)

        a = a @ v # N,H,S,D//H
        
        a = a.permute(0,2,1,3).contiguous() # N,S,H,D//H

        a = a.view(N,-1,D) # N,S,D  

        output = self.proj(a) # N,S,D 
