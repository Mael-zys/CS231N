...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h[:,0,:] = h0
    for i in range(T):
      h[:,i+1,:], cache0 = rnn_step_forward(x[:,i,:], h[:,i,:], Wx, Wh, b)
    cache = (h, x, Wx, Wh, b)
    h = h[:,1:,:]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    N, T, D = x.shape
    H = h.shape[2]
    dh_time = np.zeros((N,T+1,H))
    dx = np.zeros((N,T,D))
    dWx = np.zeros((D,H))
    dWh = np.zeros((H,H))
    db = np.zeros(H)
    for i in range(T-1,-1,-1):
      # cache_temp = (next_h, x, prev_h, Wx, Wh, b)
      cache_temp = (h[:,i+1,:], x[:,i,:], h[:,i,:], Wx, Wh, b)
      dx[:,i,:], dh_time[:,i,:], dWx_temp, dWh_temp, db_temp = rnn_step_backward(dh[:,i,:]+dh_time[:,i+1,:], cache_temp)
      dWx += dWx_temp
      dWh += dWh_temp
      db += db_temp
    dh0 = dh_time[:,0,:]
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, Wx, Wh, b, prev_h, prev_c, next_c, next_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, Wx, Wh, b, prev_h, prev_c, next_c, next_h = cache
    N, D = x.shape
    H = prev_h.shape[1]

    # calculate i, f, o, g
    # maybe it's better we save i,f,o,g in the cache
    a = x @ Wx + prev_h @ Wh + b
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
      h[:,i+1,:], c[:,i+1,:], _ = lstm_step_forward(x[:,i,:], h[:,i,:], c[:,i,:], Wx, Wh, b)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T-1,-1,-1):
      cache_step = (x[:,i,:], Wx, Wh, b, h[:,i,:], c[:,i,:], c[:,i+1,:], h[:,i+1,:])
      dx[:,i,:], dh_time[:,i,:], dc, dWx_step, dWh_step, db_step = lstm_step_backward(dh[:,i,:]+dh_time[:,i+1,:], dc, cache_step)
      dWx += dWx_step
      dWh += dWh_step
      db += db_step
